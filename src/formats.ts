...
> = {
  ...
  code: {
    label: "Code repository & files",
    badge: "Code",
    hint: `ZIP project archive, code file (.py, .js, .java, etc.) up to ${MAX_UPLOAD_LABEL}, or a GitHub link.`,
    linkHint: "https://github.com/org/repo",
    accept: ".zip,.py,.ipynb,.js,.ts,.jsx,.tsx,.html,.css,.java,.cpp,.c,.cs,.rb,.go,.rs,.php,.sql,.sh,.json,.md,.yml,.yaml,application/zip,application/x-zip-compressed,text/plain,application/json",
  },
}
